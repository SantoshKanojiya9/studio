...
const MiniFace = ({ emoji }: { emoji: EmojiState }) => {
        ...
        const eyeBase = (
            <div className="w-12 h-10 bg-fuchsia-200 rounded-full relative overflow-hidden">
                <div className="absolute top-1/2 left-1/2 w-6 h-6 bg-black rounded-full" style={{ transform: 'translate(-50%, -50%)' }}>
                    {emoji.expression === 'love' && <Heart className="w-5 h-5 text-red-500 fill-current" />}
                </div>
            </div>
        );
        switch (style) {
    ...
    const renderEyebrow = (style: typeof emoji.eyebrowStyle, isRight?: boolean) => {
        ...
        switch (style) {
            case 'male-1': return <motion.div className="absolute -top-3 left-0 w-14 h-4 bg-black" style={{ ...baseStyle, clipPath: 'polygon(0 0, 100% 20%, 90% 100%, 10% 100%)' }} variants={eyebrowVariants} animate={emoji.expression} />;
            case 'male-2': return <motion.div className="absolute -top-4 left-0 w-12 h-5 bg-black" style={{ ...baseStyle, borderRadius: '4px' }} variants={eyebrowVariants} animate={emoji.expression} />;
            case 'male-3': return <motion.div className="absolute -top-2 left-0 w-12 h-3 bg-black" style={baseStyle} variants={eyebrowVariants} animate={emoji.expression} />;
            case 'female-1': return <motion.div className="absolute -top-4 left-0 w-12 h-3 bg-black" style={{ ...baseStyle, clipPath: 'path("M0,10 C10,0 40,0 50,10")' }} variants={eyebrowVariants} animate={emoji.expression} />;
            case 'female-2': return <motion.div className="absolute -top-3 left-0 w-12 h-2.5 bg-black" style={baseStyle} variants={eyebrowVariants} animate={emoji.expression} />;
            case 'female-3': return <motion.div className="absolute -top-3 left-0 w-12 h-4 bg-black/80" style={{ ...baseStyle, clipPath: 'polygon(0 50%, 100% 0, 100% 100%, 0 100%)' }} variants={eyebrowVariants} animate={emoji.expression} />;
            default: return <motion.div className="absolute -top-3 left-0 w-12 h-4 bg-black" style={baseStyle} variants={eyebrowVariants} animate={emoji.expression} />;
        }
    };
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full h-full flex items-center justify-center">
                <motion.div className="flex justify-between w-56 absolute top-40">
                    <motion.div className="w-12 h-6 bg-pink-400 rounded-full" variants={blushVariants} animate={emoji.expression} />
                    <motion.div className="w-12 h-6 bg-pink-400 rounded-full" variants={blushVariants} animate={emoji.expression} />
                </motion.div>
                <motion.div className="flex gap-20 absolute top-28">
                    <motion.div className="relative" variants={eyeVariants} animate={emoji.expression}>
                ...
                {emoji.showSunglasses && (
                    <div className="absolute flex justify-center w-full" style={{ top: '110px' }}>
                ...
                {emoji.showMustache && (
                    <div className="absolute flex justify-center w-full" style={{ top: '170px' }}>
             ...
             <motion.div className="absolute bottom-[-40px] w-[90%] left-[5%] pb-2 h-16">
