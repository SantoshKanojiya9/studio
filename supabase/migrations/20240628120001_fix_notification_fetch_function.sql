...
-- Function to get notifications for a user, handling optional emoji data
...
begin
    ...
    select
        ...
        n.emoji_id,
        n.is_read,
        -- Aggregate actor info into a single JSON object
        json_build_object(
            'id', u.id,
            'name', u.name,
            'picture', u.picture,
        ...
        -- Aggregate emoji info into a single JSON object, only if emoji_id is present
        case
        ...
        s.status::text as actor_support_status
    from
        public.notifications n
        join public.users u on n.actor_id = u.id
        left join public.emojis e on n.emoji_id = e.id
        left join public.supports s on s.supporter_id = p_recipient_id and s.supported_id = n.actor_id
    where
