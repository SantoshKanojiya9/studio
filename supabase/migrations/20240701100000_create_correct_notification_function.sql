...
-- Drop the old, unused function if it exists, ensuring a clean state.
drop function if exists public.purge_old_notifications();

-- Create the new, correct version of the function to fetch notifications.
...
begin
    ...
    where
        ...
        and n.created_at >= now() - interval '24 hours' -- Filters out old notifications
    order by
        n.created_at desc
    limit p_limit
    offset p_offset;
