
create or replace function get_notifications_for_user(
    p_recipient_id uuid,
    p_limit int,
    p_offset int
)
returns table (
    id bigint,
    type public.notification_type,
    created_at timestamptz,
    emoji_id uuid,
    actor json,
    emoji json,
    actor_support_status public.support_status
)
language plpgsql
as $$
begin
    return query
    with notification_data as (
        select
            n.id,
            n.type,
            n.created_at,
            n.emoji_id,
            n.actor_id,
            n.recipient_id
        from public.notifications n
        where n.recipient_id = p_recipient_id
        and n.created_at >= now() - interval '24 hours'
        order by n.created_at desc
        limit p_limit
        offset p_offset
    )
    select
        nd.id,
        nd.type,
        nd.created_at,
        nd.emoji_id,
        json_build_object(
            'id', u.id,
            'name', u.name,
            'picture', u.picture,
            'is_private', u.is_private
        ) as actor,
        (
            select json_build_object(
                'id', e.id,
                'created_at', e.created_at,
                'user_id', e.user_id,
                'model', e.model,
                'expression', e.expression,
                'background_color', e.background_color,
                'emoji_color', e.emoji_color,
                'show_sunglasses', e.show_sunglasses,
                'show_mustache', e.show_mustache,
                'selected_filter', e.selected_filter,
                'animation_type', e.animation_type,
                'shape', e.shape,
                'eye_style', e.eye_style,
                'mouth_style', e.mouth_style,
                'eyebrow_style', e.eyebrow_style,
                'feature_offset_x', e.feature_offset_x,
                'feature_offset_y', e.feature_offset_y,
                'caption', e.caption
            ) from public.emojis e where e.id = nd.emoji_id
        ) as emoji,
        (
            select s.status from public.supports s
            where s.supporter_id = p_recipient_id and s.supported_id = nd.actor_id
        ) as actor_support_status
    from notification_data nd
    join public.users u on nd.actor_id = u.id;
end;
$$;
